...
productRouter.post('/:id/reviews', isAuth, expressAsyncHandler(async(req, res) => {
    ...
    if(product) {
        if(product.reviews.find(x => x.name === req.user.name)) {
            return res.status(400).send({message: 'You have already submitted a review!'})
        }
        ...
        res.status(201).send({message: 'Reviewf Created!', review: updatedReview.reviews[updatedReview.reviews.length -1]});
    } else {
        res.status(404).send({message: 'Product Not Found!'})
    }
}));
